# Description: Fetches revenue estimate data for a list of tickers from the
...
# Fetch the revenue estimates data
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_revenue_estimate.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_revenue_estimate_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
