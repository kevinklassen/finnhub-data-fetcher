# Description: Fetches quarterly balance sheet data for a list of tickers from
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_financials_bs_q.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_financials_bs_q_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
