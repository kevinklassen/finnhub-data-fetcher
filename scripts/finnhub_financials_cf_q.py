# Description: Fetches quarterly cash flow statement data for a list of tickers
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_financials_cf_q.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_financials_cf_q_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
