# Description: Fetches annual balance sheet data for a list of tickers from
# the Finnhub API and saves the data to a CSV file.
import asyncio
import os
import pandas as pd
import random
...
# Fetch the annual balance sheet data
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_financials_bs_a.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_financials_bs_a_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
