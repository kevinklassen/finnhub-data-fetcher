# Description: Fetches annual cash flow statement data for a list of tickers
# from the Finnhub API and saves the data to a CSV file.
import asyncio
import os
import pandas as pd
import random
...
# Fetch the annual cash flow statement data
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_financials_cf_a.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_financials_cf_a_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
