# Description: Fetches EBITDA estimate data for a list of tickers from the
...
# Fetch the EBITDA estimate data
results_df, api_call_timestamps = asyncio.run(
    fetch_finnhub_data(
        tickers,
        endpoint="ebitda-estimate",
...
# Save Results to CSV
results_df.to_csv(
    os.path.join(output_folder, "finnhub_ebitda_estimate.csv"), index=False
)
...
api_call_timestamps_df.to_csv(
    os.path.join(
        output_folder,
        f"finnhub_ebitda_estimate_api_call_timestamps_{pd.Timestamp.now().date()}.csv",
    ),
    index=False,
)
